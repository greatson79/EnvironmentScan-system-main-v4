...
| DEC-025 | Task Management Python 원천봉쇄 | Deterministic task creation must be Python-enforced, not LLM-paraphrased |
| DEC-026 | Dashboard Bilingual Display + Signal Map | Bilingual Protocol mandates Korean for external output; dashboard was English-only |

---

**Document Version**: 6.0
**Last Updated**: 2026-03-29
**System Version**: Quadruple Workflow System v3.6.0
...
## DEC-026: Dashboard Bilingual Display + Signal Map Visualization

**Date**: 2026-03-29
**Context**: Dashboard Top 20 and all WF summary tables displayed English-only titles despite WF3/WF4 classified-signals containing title_ko. The title_ko field was generated by emergent LLM behavior (not explicitly mandated), and build_top_signals() failed to extract it due to WF1 using "items" key and nested classification.* fields.

**Decision**:
1. Mandate title_ko in phase2-analyst.md output schema (Section C-2) for ALL workflows
2. Fix build_top_signals() to handle WF-specific data structures (WF1 nested, WF2 different field names)
3. Add PG2-009 (title_ko Korean presence check) to validate_phase2_output.py — Python 원천봉쇄
4. Add narrative EN/KO subtabs (KO default) for Patterns, Strategic, Scenario sections
5. Add D3.js interactive signal map in timeline tab (STEEPs x Impact, lazy-init for hidden tabs)
6. Normalize WF1 impact scores (0-100 → 0-10) for cross-WF consistency
7. Enforce validate_phase2_output.py for WF2 Pipeline Gate 2 (was LLM-only)

**Rationale**: Bilingual Protocol (Rule 2.6) mandates Korean for external output. Dashboard is external output but displayed English-only. Translation at Phase 2 (with full signal context) produces highest quality; dashboard pipeline stays 100% Python (원천봉쇄).

**Impact**: phase2-analyst.md, dashboard_data_extractor.py, dashboard_generator.py, validate_phase2_output.py, 4 orchestrators. Zero pipeline stage changes. Zero SOT changes.
